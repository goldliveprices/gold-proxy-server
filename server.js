'use strict';
// ═══════════════════════════════════════════════════════════════════
// RR JEWELLERS GOLD SERVER — Dhan Live Feed v3
// FIX: Correct WebSocket URL with token+clientId in query params
// FIX: RenewToken auto-refresh every 20hr
// FIX: Live USD/INR + XAU/USD cached every 5 min
// ═══════════════════════════════════════════════════════════════════
...
// Hardcoded fallback tokens — auto-updated from Dhan API on startup
let TOKENS = {
...
const forexCache = {
  usdInr:94.5, xauUsd:3310, xagUsd:32.8, updatedAt:null, src:'init',
};
...
async function refreshForexAndSpot() {
  ...
  // USD/INR
  const fxSources=[
    ()=>axios.get('https://api.frankfurter.app/latest?from=USD&to=INR',{timeout:5000}).then(r=>r.data?.rates?.INR),
    ()=>axios.get('https://open.er-api.com/v6/latest/USD',{timeout:5000}).then(r=>r.data?.rates?.INR),
    ()=>axios.get('https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/usd.json',{timeout:5000}).then(r=>r.data?.usd?.inr),
  ];
  const fxNames=['frankfurter','open.er-api','fawazahmed0'];
  for (let i=0;i<fxSources.length&&!usdInr;i++) {
    try { const v=await fxSources[i](); if(v>70&&v<110){usdInr=v;src=fxNames[i];} } catch {}
  }
...
async function renewDhanToken() {
  if (!currentAccessToken||!DHAN_CLIENT_ID) {
    console.warn('[TOKEN] Cannot renew — missing token/clientId'); return false;
  }
  try {
    console.log('[TOKEN] Calling RenewToken...');
    ...
    if (newToken) {
      ...
      console.log('[TOKEN] Renewed — valid 24hr from',tokenRenewedAt);
...
// ─── BINARY PACKET PARSER (Dhan v2 spec) ──────────────────────────
// Header: byte[0]=feedCode, byte[1-2]=msgLen, byte[3]=exchSeg, byte[4-7]=secId(int32LE)
// Ticker: bytes 8-11=LTP(float32LE), 12-15=LTT(int32LE)
// Quote:  bytes 8-11=LTP, 12-19=LTT+OI, 20-27=open, 28-35=high, 36-43=low,
//         44-51=close, 52-59=vol, 60-63=bid, 64-67=ask (approx offsets)
function parseDhanPacket(buf) {
  try {
    if (!buf||buf.length<12) return null;
    const feedCode=buf.readUInt8(0);
    // secId is at bytes 4-7 (int32 LE) per Dhan v2 header spec
    const secId=buf.readInt32LE(4).toString();

    // Ticker packet (feedCode=2)
    if (feedCode===2&&buf.length>=12) {
      const ltp=Math.round(buf.readFloatLE(8)/100);
      if(ltp<=100||ltp>999999) return null;
      return {secId,ltp,bid:ltp,ask:ltp,high:0,low:0,open:0,mode:'ticker'};
    }

    // Quote packet (feedCode=4)
    if (feedCode===4&&buf.length>=50) {
      const ltp=Math.round(buf.readFloatLE(8)/100);
      if(ltp<=100||ltp>999999) return null;
      // Quote offsets from Dhan v2 docs
      const open=buf.length>28?Math.round(buf.readFloatLE(20)/100):0;
      const high=buf.length>32?Math.round(buf.readFloatLE(28)/100):0;
      const low =buf.length>36?Math.round(buf.readFloatLE(36)/100):0;
      let bid=ltp,ask=ltp;
      if(buf.length>64){
        const b=Math.round(buf.readFloatLE(60)/100);
        const a=Math.round(buf.readFloatLE(64)/100);
        if(b>0)bid=b; if(a>0)ask=a;
      }
      return {secId,ltp,bid,ask,high,low,open,mode:'quote'};
    }

    // Full packet (feedCode=5) — similar structure, more fields
    if (feedCode===5&&buf.length>=80) {
      const ltp=Math.round(buf.readFloatLE(8)/100);
      if(ltp<=100||ltp>999999) return null;
      const open=Math.round(buf.readFloatLE(20)/100)||0;
      const high=Math.round(buf.readFloatLE(28)/100)||0;
      const low =Math.round(buf.readFloatLE(36)/100)||0;
      let bid=ltp,ask=ltp;
      if(buf.length>64){
        const b=Math.round(buf.readFloatLE(60)/100);
        const a=Math.round(buf.readFloatLE(64)/100);
      ...
      return {secId,ltp,bid,ask,high,low,open,mode:'full'};
    }
    return null;
  } catch {return null;}
}

// ─── WEBSOCKET — CORRECT URL FORMAT ───────────────────────────────
// Dhan v2 docs: wss://api-feed.dhan.co?version=2&token=X&clientId=Y&authType=2
function getDhanWsUrl() {
  const token=encodeURIComponent(currentAccessToken);
  const clientId=encodeURIComponent(DHAN_CLIENT_ID);
  return `wss://api-feed.dhan.co?version=2&token=${token}&clientId=${clientId}&authType=2`;
}
...
function connectDhan() {
  ...
  const wsUrl=getDhanWsUrl();
  console.log('[WS] Connecting with v2 URL (token+clientId in params)...');
  ...
  ws.on('open',()=>{
    ...
    // Subscribe to Quote mode (RequestCode 15=Ticker, 16=Quote, 17=Full)
    const instruments=[
      TOKENS.goldCurrent.secId,TOKENS.goldNext.secId,
      TOKENS.silverCurrent.secId,TOKENS.silverNext.secId,
    ].filter(Boolean).map(id=>({ExchangeSegment:'MCX_FO',SecurityId:id}));

    ws.send(JSON.stringify({
      RequestCode:17,  // Full packet = LTP + OHLC + Bid/Ask
      InstrumentCount:instruments.length,
      InstrumentList:instruments,
    }));
    console.log('[WS] Subscribed',instruments.length,'MCX instruments (Full mode)');
  });
  ...
  ws.on('message',(data)=>{
    if(typeof data==='string'){
      try{
        ...
        console.log('[WS] Text msg:',JSON.stringify(msg).slice(0,120));
      }catch{}
      return;
    }
  ...
  ws.on('close',(code,reason)=>{
    ...
    const msg=reason?.toString()?.slice(0,80)||'';
    console.log('[WS] Disconnected code:',code,msg);
    // 400=bad request(URL issue), 401/4001=auth fail
    if(code===400){
      console.error('[WS] HTTP 400 — check URL format or token');
      scheduleReconnect();
    } else if(code===401||code===4001||msg.toLowerCase().includes('unauthori')){
      console.log('[WS] Auth error — renewing token');
...
app.get('/debug',(req,res)=>res.json({
  server:'RR Jewellers Gold Server — Dhan v3',wsStatus:WS.wsStatus,
  ...
  wsUrlPreview:`wss://api-feed.dhan.co?version=2&token=[hidden]&clientId=${DHAN_CLIENT_ID}&authType=2`,
...
app.get('/',(req,res)=>res.json({
  status:'RR Jewellers Gold Server — Dhan v3',wsStatus:WS.wsStatus,
  endpoints:['/rates','/debug','/spot-test','/forex-test','/cache-status','/ping','/token-renew','/updates'],
}));

// ─── STARTUP ───────────────────────────────────────────────────────
app.listen(PORT,'0.0.0.0',async()=>{
  console.log(`[STARTUP] RR Jewellers Gold Server v3 — port ${PORT}`);
  await refreshForexAndSpot();           // Forex immediately
  await fetchDhanInstruments();          // Instrument tokens
  connectDhan();                         // WebSocket (correct URL)

  setInterval(refreshForexAndSpot,5*60*1000);    // Forex every 5 min
  setInterval(resetSessionIfNewDay,60*1000);      // Session reset
  setInterval(()=>axios.get((SELF_URL||`http://localhost:${PORT}`)+'/ping').catch(()=>{}),4*60*1000); // Keep-alive

  // Token auto-renew every 20 hours
  setInterval(async()=>{
    console.log('[TOKEN] Auto-renew (20hr)');
    await renewDhanToken();
  },20*60*60*1000);

  // WS health check every 2 min
  setInterval(()=>{
