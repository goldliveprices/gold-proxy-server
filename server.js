...
// optional / future use (abhi use nahi ho rahe)
...
// Month map (contracts ke display ke liye)
const MONTHS   = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'];
const GOLD_M   = [0, 2, 4, 6, 8, 10];
const SILVER_M = [1, 3, 5, 7, 10];

// Feed response codes (v2 docs)
const FEED_CODE = {
  INDEX:         1,
  TICKER:        2,
  QUOTE:         4,
...
// Hardcoded fallback; startup pe instruments API se overwrite hoga
let TOKENS = {
...
function isMCXOpen() {
  ...
  const t = hour * 60 + min;
  // Sat: 9:00–14:00, Mon–Fri: 9:00–23:55 (approx, tumhara pehle ka logic)
...
function buildContracts() {
  ...
  return {
    gold: {
      current: (gC[0]?.month || '') + (gC[0]?.year || ''),
      next:    (gC[1]?.month || '') + (gC[1]?.year || ''),
    },
    silver: {
      current: (sC[0]?.month || '') + (sC[0]?.year || ''),
      next:    (sC[1]?.month || '') + (sC[1]?.year || ''),
    },
  };
}
...
function getSpotDerived() {
  ...
  const F = 1.103; // making charges + gst approx
  return {
...
async function renewDhanToken() {
  ...
  try {
    ...
    console.warn('[TOKEN] renew: no token in response');
    return false;
  } catch (e) {
...
// ─── INSTRUMENT LOOKUP (MCX futures only) ─────────────────────────
...
');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const name = (c[nameCol] || '').toUpperCase();
      const sym  = (c[symCol]  || '').toUpperCase();
      const item = { secId: c[secIdCol], symbol: sym, expiry: c[expiryCol] || '' };

      // Only main GOLD / SILVER futures (no mini/micro)
    ...
    const now  = new Date();
    const sort = arr => arr
      .filter(x => new Date(x.expiry) > now)
      .sort((a, b) => new Date(a.expiry) - new Date(b.expiry));

    const gS = sort(gold);
    const sS = sort(silver);

    if (gS.length >= 2 && sS.length >= 2) {
      TOKENS.goldCurrent   = gS[0];
      TOKENS.goldNext      = gS[1];
      TOKENS.silverCurrent = sS[0];
      TOKENS.silverNext    = sS[1];
      ...
      console.warn('[INSTRUMENTS] not enough contracts; using fallback TOKENS');
    }
  } catch (e) {
    console.warn('[INSTRUMENTS] fetch failed', e.message);
  }
}

// ─── BINARY PARSER (v2 offsets exact from docs) ────────────────────
// Header (8 bytes, 0-based):
//   [0] = feedCode (uint8)
//   [1-2] = msgLen (int16 LE)
//   [3] = exchSeg (uint8)
//   [4-7] = securityId (int32 LE)[web:4]
function parseDhanPacket(buf) {
  try {
    ...
    // Ticker: header(0-7) + LTP(8-11) + LTT(12-15)[web:4]
    ...
    // Quote packet[web:4]
    // LTP(8), LTQ(12), LTT(14), ATP(18), Vol(22),
    // totalSell(26), totalBuy(30), open(34), close(38), high(42), low(46)
    ...
    // Full packet (with depth)[web:4]
    // LTP(8), LTQ(12), LTT(14), ATP(18), Vol(22),
    // totalSell(26), totalBuy(30),
    // OI(34), OIHigh(38), OILow(42),
    // open(46), close(50), high(54), low(58),
    // depth 5 x 20 bytes from offset 62
    if (feedCode === FEED_CODE.FULL && buf.length >= 80) {
      ...
      let bid = Math.round(ltp);
      let ask = Math.round(ltp);

      // depth start = 62; first level entry, BidPrice offset 12, AskPrice offset 16[web:4]
      if (buf.length >= 82) {
        const depthStart = 62;
        const b = buf.readFloatLE(depthStart + 12) / 100;
        const a = buf.readFloatLE(depthStart + 16) / 100;
...
function connectDhan() {
  if (!DHAN_CLIENT_ID || !currentAccessToken) {
    console.warn('[WS] missing DHAN_CLIENT_ID or ACCESS_TOKEN');
    return;
  }
  ...
  const wsOptions = {};
  if (HTTP_PROXY) {
    // future: yahan proxy agent plug kar sakte ho (tunnel, https-proxy-agent etc.)
  }

  const ws = new WebSocket(getDhanWsUrl(), {
  ...
  ws.on('message', (data) => {
    if (typeof data === 'string') {
      // Dhan kabhi‑kabhi text error bhejta hai; debug ke liye
      console.log('[WS] text', data.toString().slice(0, 200));
      return;
    }
    ...
    if (sid === TOKENS.goldCurrent.secId) {
      ...
      console.log('[WS] tick for unknown secId', sid, 'ltp', tick.ltp);
    }
  });
  ...
  ws.on('close', (code, reason) => {
    ...
    console.log('[WS] closed', code, reason?.toString()?.slice(0, 100) || '');
...
function scheduleReconnect() {
  ...
  console.log('[WS] reconnect in', delay / 1000, 'sec (attempt', WS.reconnectCount, ')');
...
app.get('/rates', async (req, res) => {
  ...
  if (isDhanLive()) {
    ...
    const payload = {
      ...
      futures: {
        gold: {
          ltp: g.ltp, bid: g.bid, ask: g.ask,
          high: gH, low: gL, open: g.open,
        },
        silver: {
          ltp: s.ltp, bid: s.bid, ask: s.ask,
          high: sH, low: sL, open: s.open,
        },
        goldNext: {
          ltp:  gN.ltp  || g.ltp,
          bid:  gN.bid  || g.bid,
  ...
  return res.json({
    ...
    source: 'spot_derived',
    marketOpen,
    note: 'Live MCX unavailable — spot→INR',
    spotSource: d.src,
    usdInr: d.usdInr,
    xauUsd: d.xauUsd,
    xagUsd: d.xagUsd,
...
app.get('/debug', (req, res) => {
  res.json({
    ...
    currentSource: isDhanLive() ? 'dhan_mcx_live'
  ...
  try {
    ...
    res.json({
      ...
      updates: [{
        ...
        content: 'Live gold & silver rates.',
        image: '',
      }],
    });
  }
...
app.get('/', (req, res) => res.json({
  status: 'RR Jewellers — Dhan v4-final',
  wsStatus: WS.wsStatus,
  endpoints: [
    '/rates',
    '/debug',
