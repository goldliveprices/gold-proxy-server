...
// Write MCX tick — returns true if ltp changed OR first data
// RC15 (Ticker): only ltp arrives, bid/ask/ohlc = 0 — still broadcast
// RC17 (Quote):  ltp + ohlc arrive — broadcast on ltp change
function writeMCX(key, tick) {
  ...
  if (!fields || !tick.ltp || tick.ltp <= 0) return false;

  const prevLtp = state[fields[0]];
  // Always update all available fields
  state[fields[0]] = tick.ltp;
  if (fields[1] && tick.bid  > 0) state[fields[1]] = tick.bid;
  if (fields[2] && tick.ask  > 0) state[fields[2]] = tick.ask;
  if (fields[3] && tick.high > 0) state[fields[3]] = tick.high;
  if (fields[4] && tick.low  > 0) state[fields[4]] = tick.low;
  if (fields[5] && tick.open > 0) state[fields[5]] = tick.open;
  if (fields[6] && tick.prevClose > 0) state[fields[6]] = tick.prevClose;

  // Broadcast if: ltp changed OR first data (prevLtp was 0)
  const changed = (tick.ltp !== prevLtp) || (prevLtp === 0);
  if (changed) {
...
function writeFX(sym, data) {
  ...
  if (sym === 'XAU' && data.price > 3000 && data.price < 9000) {
    if (data.price > 0 && data.price !== state.xauUsd) { state.xauUsd = round(data.price, 2); changed = true; }
    else if (data.price === 0 && state.xauUsd === 0) changed = false; // H/L only update, no price change
    ...
    if (data.price > 0 && data.price !== state.xagUsd) { state.xagUsd = round(data.price, 3); changed = true; }
    ...
    if (data.price > 0 && data.price !== state.usdInr) { state.usdInr = round(data.price, 2); changed = true; }
